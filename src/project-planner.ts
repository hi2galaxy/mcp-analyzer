...
export function analyzeTechStack(description: string): TechnologyStack {
  ...
  // Detect languages
  if (
    description.match(/typescript|ts|angular|react|next\.?js|vue|node\.?js/i)
  ) {
    techStack.languages.push("TypeScript");
  }
  if (
    description.match(/javascript|js|node\.?js|express|react|vue/i) &&
    !description.match(/typescript|ts/i)
  ) {
  ...
  if (
    description.match(
      /auth|authentication|login|user|permission|role|jwt|oauth/i
    )
  ) {
  ...
  if (
    description.match(
      /ci|cd|ci\/cd|continuous\s*integration|continuous\s*deployment|github\s*actions|jenkins/i
    )
  ) {
...
async function generateStarterFiles(
  ...
  if (
    techStack.languages.includes("TypeScript") ||
...
`;
    ...
    await fs.writeFile(path.join(projectPath, "main.py"), mainContent);
  }
}
